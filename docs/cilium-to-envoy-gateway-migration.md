...
- [x] Configure separate DNS names to avoid conflicts
  - `envoy-internal.${SECRET_DOMAIN}` (192.168.123.140)
  - `envoy-external.${SECRET_DOMAIN}` (192.168.123.141)
...
1. Update DNS to point to Envoy gateways
   - `internal.${SECRET_DOMAIN}` → 192.168.123.140 (envoy-internal)
   - `external.${SECRET_DOMAIN}` → 192.168.123.141 (envoy-external)
...
# Check DNS records
nslookup <app>.${SECRET_DOMAIN} <unifi-dns-ip>
```
