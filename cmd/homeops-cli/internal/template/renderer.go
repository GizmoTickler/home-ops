...
func (r *Renderer) injectSecrets(content []byte) ([]byte, error) {
	result, err := r.metrics.TrackOperationWithResult("secret_injection", func() (interface{}, error) {
		...
		replaced := opRegex.ReplaceAllFunc(content, func(fullMatch []byte) []byte {
			key := string(fullMatch)
			if v, ok := cache[key]; ok {
				return v
			}
			// Extract capture groups for vault/item/field
			sub := opRegex.FindSubmatch(fullMatch)
			if len(sub) != 4 { // full + 3 groups
				// Should not happen; leave unchanged to be caught later
				return fullMatch
			}
			vault := string(sub[1])
			item := string(sub[2])
			...
			secret, getErr := r.getSecret(vault, item, field)
			if getErr != nil {
				// Leave unresolved to surface an error after replacement
				return fullMatch
			}
			b := []byte(secret)
			cache[key] = b
		...
		// If any references remain, return a structured error
		if opRegex.Match(replaced) {
			rem := opRegex.Find(replaced)
			return nil, errors.NewTemplateError("SECRET_INJECTION_FAILED",
				fmt.Sprintf("Unresolved 1Password reference: %s", string(rem)), nil)
		}
